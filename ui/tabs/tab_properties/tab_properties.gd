	...
	for property_name in properties:
		...
		match property.hint:
			...
			Element.Hint.BLUEPRINT:
				field = BlueprintField.SCENE.instantiate().init(property_container, property_name, property)
				field.value_changed.connect(_on_blueprint_changed)
		...
		if element is Shape and property_name == "frame":
			field.set_param("max_value", element.texture_attributes.get("frames", 1) - 1)
...
func _on_field_value_changed(property_name: String, new_value: Variant):
	if element_selected.blueprint:
		element_selected.blueprint.set_property(property_name, new_value)
		if not unsaved_blueprints.has(element_selected.blueprint):
			unsaved_blueprints.append(element_selected.blueprint)
		save_blueprints()
	else:
		...
		if element_selected is Shape and property_name == "texture":
			_set_element_selected(element_selected)
